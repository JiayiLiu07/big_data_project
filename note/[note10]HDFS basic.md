...
### 文件系统是什么？
- 操作对象（人类）和硬盘的中间层
- 想象一下：假设没有文件系统，从网上下载的数据包该怎么存放？放到哪儿里
    - 放到硬盘 
    - 但是网络传输是通过进程实现的，不能直接落盘（一个进程占用一个内存）
    - 所以文件系统的功能就是解决了我们看不到的：把数据包存放在硬盘的哪儿个区域
    - ps：一般从网上下载东西是直接下载到Download文件夹，但是现在就是没了文件夹。所以通过类似用终端从远程拉取文件的方式（命令）下载文件。
- 功能：
    - 把硬盘抽象成我们能看到的文件夹
    - 区分不同文件类型（这只用调整后缀名就能简单做到）
    - 权限管理
    - 等等
